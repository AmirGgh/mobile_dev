...
export default function CoachOnboarding() {
  ...
  const [subgroups, setSubgroups] = useState([{ name: '', sport: 'טריאתלון', level: 'מתחילים', goal: '', days: [] as string[] }]);
  ...
  const finishOnboarding = async () => {
    ...
    const { data: group } = await supabase
      .from('groups')
      .insert({ name: clubName, head_coach_id: user.id })
      .select()
      .single();
  ...
  const renderStep1 = () => (
    <View className="flex-1 justify-center pb-20">
      <View className="bg-neutral-900/80 rounded-[32px] p-8 border border-neutral-800">
        <Text className="text-white text-3xl font-black text-center mb-2">הרשמה כמאמן</Text>
        ...
        <View className="mb-10">
          <Text className="text-neutral-400 text-sm mb-2 text-right font-medium">שם הקבוצה</Text>
          <TextInput
            className="bg-neutral-950 border border-neutral-800 rounded-2xl h-16 px-5 text-white text-right text-base"
        ...
        <TouchableOpacity 
          ...
          className="bg-[#22c55e] rounded-2xl h-16 items-center justify-center shadow-lg shadow-green-900/50"
          activeOpacity={0.8}
        >
          <Text className="text-neutral-950 font-bold text-lg">המשך</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  const renderStep2 = () => (
    <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      {subgroups.length < 5 && (
        <TouchableOpacity 
          onPress={() => setSubgroups([...subgroups, { name: '', sport: 'טריאתלון', level: 'מתחילים', goal: '', days: [] }])}
      ...
      <TouchableOpacity 
        onPress={nextStep}
        className="bg-[#22c55e] rounded-2xl h-16 items-center justify-center mt-4"
        activeOpacity={0.8}
      >
        <Text className="text-neutral-950 font-bold text-lg">המשך לסיכום ותשלום</Text>
      </TouchableOpacity>
    </ScrollView>
  );
  ...
  const renderStep3 = () => (
    <View className="flex-1 justify-center pb-20">
      ...
      <View className="bg-neutral-900/80 border border-neutral-800 rounded-[32px] p-8 mb-10">
        ...
        <Text className="text-neutral-400 text-right mb-6 text-base">מספר תתי-קבוצות מנוהלות: {subgroups.length}</Text>
